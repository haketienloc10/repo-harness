...
| 5   | Task state             | Covered | `scripts/bin/harness-cli query matrix`, `scripts/bin/harness-cli query status`, `docs/TEST_MATRIX.md`, `intake` table, `story` table, `trace` table, `story.next_action`/`trace.next_action` (migration 006)                                                                                                                                                          | Durable records track intake, story status, proof columns, and task traces; `next_action` carries WIP across sessions and surfaces in `query status` RESUME.                                                                          | Lifecycle checks now exist via `done-check`; benchmark of resume continuity is future work.   |
| 6   | Observability          | Partial | `docs/TRACE_SPEC.md`, `docs/product/read-model.md`, `trace` table, `scripts/bin/harness-cli trace`, `scripts/bin/harness-cli score-trace`, `scripts/bin/harness-cli query traces`, `scripts/bin/harness-cli query friction`, `scripts/bin/harness-cli query status`, `scripts/bin/harness-cli query recap`, `docs/HARNESS_MATURITY.md`                              | Traces are auto-scored when recorded; the Read-Model (`query status`/`query recap`) gives a deterministic ranked snapshot of being/been/to-do plus per-story rollup over traces.                                                      | Read-Model is a derived view; benchmark/attribution ingestion (full H3) remains future work.  |
...
| 8   | Verification           | Covered | `docs/TEST_MATRIX.md`, `scripts/bin/harness-cli query matrix`, `scripts/bin/harness-cli story verify`, `scripts/bin/harness-cli story verify-all`, `scripts/bin/harness-cli done-check`, `scripts/bin/harness-cli evidence`, `evidence` table (migration 007), `story.verify_command`, `story.last_verified_result`, `.github/workflows/harness-cli-release.yml`, `docs/templates/validation-report.md` | Proof commands run individually or in batch; `story verify` auto-captures a durable evidence log so a proof boolean always has a fresh log; `done-check` packages lane-aware completion gates with an exit code.                       | Benchmark ingestion remains future work.                                                      |
...
| `scripts/schema/005-tool-extensions.sql`                                      | Tool access            | Verification                                           |
| `scripts/schema/006-next-action.sql`                                          | Task state             | Observability                                          |
| `scripts/schema/007-evidence.sql`                                             | Verification           | Observability, project memory                          |
